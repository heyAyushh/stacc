...
- Task layer identified (UI, client, program, testing, infra).
- RPC/cluster, fee payer, blockhash, and compute budget are explicit.
- Account owners/signers/writability verified for each instruction.
- Token program variant (SPL vs Token-2022) is correct.
- Tests added (LiteSVM/Mollusk for unit; Surfpool for integration when needed).
- Security checklist applied for signing/fees/CPIs and token transfers.
