...
## Objective
Babysit a PR for the user-approved monitoring window, until one of these
terminal outcomes occurs:

- The PR is merged or closed.
- A situation requires user help (for example CI infrastructure issues, repeated flaky failures after retry budget is exhausted, permission problems, or ambiguity that cannot be resolved safely).
- Optional handoff milestone: the PR is currently green + mergeable + review-clean. Treat this as a progress state unless the agreed monitoring window ends.
...
1. Agree on a monitoring window or deadline. When the host supports a recurring monitor, use it for monitoring beyond the current task; otherwise use `--watch` only for the agreed bounded window. Use a one-shot snapshot when the user asks for a diagnostic check.
...
13. If the agreed monitoring window remains active after a patch/commit/push, relaunch `--watch` for the rest of that window.
14. Repeat polling until `stop_pr_closed` appears, a user-help-required blocker is reached, or the agreed window ends. A green + review-clean + mergeable PR is a progress milestone.
15. Maintain terminal/session ownership while that monitoring window is active; do not leave a detached `--watch` process running.
...
6. If the agreed monitoring window remains active, restart `--watch` after the push for the remainder of that window.
...
- If you interrupted a live `--watch` session to make the fix, restart it after the push only while the agreed monitoring window remains active.
- Do not run multiple concurrent `--watch` processes for the same PR/state file; keep one watcher session active and reuse it until its window ends or you intentionally restart it.
- A push is not a terminal outcome; continue the monitoring loop while the agreed monitoring window remains active.
...
9. If you pushed a commit, resolved a review thread, or triggered a rerun, report the action briefly and continue polling while the agreed window remains active. If a human review comment needs a written GitHub response, stop and ask for confirmation before posting.
10. After a review-fix push, restart continuous monitoring (`--watch`) only for the remainder of the agreed window.
11. If everything is passing, mergeable, not blocked on required review approval, and there are no unaddressed review items, report that the PR is currently ready to merge. Continue only for the remaining agreed window.
...
When the user explicitly asks to monitor/watch/babysit a PR, agree on a window and prefer `--watch` within it. Use a host recurring monitor for monitoring that must outlive the current task when available. Use repeated `--once` snapshots for debugging, local testing, or one-shot checks.
Do not leave a detached watcher running. At the end of the agreed window, report the current state and offer another window or a recurring monitor if the PR is still open.
After a review-fix push, restarting the watcher for the remainder of the same window is part of the babysitting task.
...
- After CI turns green: keep polling at the base cadence for the remainder of the agreed monitoring window.
...
- The agreed monitoring window ends.
...
- CI is green and mergeable, but the agreed monitoring window still has time remaining.
- The PR is green but blocked on review approval (`REVIEW_REQUIRED` / similar) and the agreed monitoring window still has time remaining.

## Output Expectations
Provide concise progress updates while monitoring and a final summary that includes:

- During long unchanged monitoring periods, avoid emitting a full update on every poll; summarize only status changes plus occasional heartbeat updates.
- Treat push confirmations, intermediate CI snapshots, ready-to-merge snapshots, and review-action updates as progress updates while the agreed monitoring window remains active.
- A user request to "monitor" is not satisfied by a couple of sample polls when a longer window was agreed; remain in the loop until a stop condition.
- A review-fix commit + push is not a completion event; resume live monitoring (`--watch`) for the remainder of the same window.
...
- Do not send the final summary while a watcher terminal is still running within its agreed window; otherwise report the current state at the end of the window.
