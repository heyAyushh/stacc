...
### 2. Missing TTY Writability Check (Line 689-695)
**Severity: HIGH**
...
### 3. Terminal State Not Restored on Error (Line 497-528)
**Severity: HIGH**
...
### 4. Temp File Leak on Error (Line 1089)
**Severity: MEDIUM**
...
### 5. Whitespace in Categories Not Handled (Line 1184)
**Severity: MEDIUM**
...
### 6. Empty PROJECT_ROOT Risk (Line 8)
**Severity: MEDIUM**
...
### 7. Division by Zero Possibility (Line 351)
**Severity: LOW**
...
### 8. Substring Matching Bug in Editor Selection (Line 820-822)
...
### 9. Unquoted Pattern in Parameter Expansion (Line 1052)
**Severity: LOW**
...
### 10. Inconsistent CONFLICT_MODE Handling (Line 1079-1086)
**Severity: LOW**
**Location:** `merge_mcp()` function
...
### 11. Dry-Run Mode Doesn't Handle MCP Merge (Line 1090)
**Severity: LOW**
**Location:** `merge_mcp()` function

**Issue:** In dry-run mode, the script attempts to create a temp file for MCP merging even though the merge won't execute. This creates an unnecessary empty temp file that gets leaked (not cleaned by the EXIT trap).
...
- Shell creates/truncates `"${tmp}"` (empty file) before run_cmd executes
- `run_cmd` just prints the jq command (doesn't run it)
- Line 1095: `run_cmd mv "${tmp}" "${dest}"` prints but doesn't execute (dest is safe)
- Empty temp file is left behind (minor leak)

**Note:** The existing MCP config is NOT corrupted in dry-run mode because `run_cmd mv` doesn't execute. However, the function should exit early in dry-run mode to avoid creating temp files unnecessarily.
...
**High:** 3 bugs (TTY checks, terminal state, temp leak on error)
**Medium:** 5 bugs (whitespace, empty pwd, division, matching, conflict mode)
**Low/Trivial:** 9 issues (dry-run temp leak, typos, unused vars, compatibility)

**Most Urgent Fixes:**
1. Validate category names (security - CRITICAL)
2. Add TTY writability check (UX - prevents crashes)
3. Restore terminal state on error (UX - prevents broken terminal)
4. Fix temp file leak in merge_mcp on error (cleanup)
